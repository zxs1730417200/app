  ...
  <div>
    ...
    <div class="main">
      <div class="py-container">
        ...
        <!--selector-->
        <SearchSelector @trademarkList="trademarkList" @props="propsList"/>
            ...
            <div class="navbar-inner filter">
              <ul class="sui-nav">
                <li :class="{active:isOne}" @click="changeOder(1)">
                  <a href="#">综合<span v-show="isOneUp">﹀</span><span v-show="isOneDown">︿</span></a>
                </li>
                <li :class="{active:isTwo}" @click="changeOder(2)">
                  <a href="#">价格<span v-show="isUp">﹀</span><span v-show="isDown">︿</span></a>
                </li>
              </ul>
            </div>
          </div>
          <div class="goods-list">
            <ul class="yui3-g">
              <li class="yui3-u-1-5" v-for="item in goodsList" :key="item.id">
                <div class="list-wrap">
                  <div class="p-img">
                    <a href="item.html" target="_blank" @click.prevent="godetail(item.id)"><img :src="item.defaultImg" /></a>
                  </div>
                  <div class="price">
                    <strong>
                      <em>¥</em>
...
<script>
  ...
  export default {
    ...
    computed:{
      ...
      ...mapState({
        total:state=>state.search.reqGetSearchInfo.total
      })
    },
    methods:{
      getData(params){
        this.$store.dispatch('reqGetSearchInfo',params)
      },
      removecategoryName(){
      ...
      trademarkList(trademark){
        let trademarkobj=`${trademark.tmId}:${trademark.tmName}`
        this.searchParams.trademark=trademarkobj
        this.getData(this.searchParams)
      },
      removetrademark(){
      ...
      changeOder(flag){
        ...
        let initialsort=this.searchParams.order.split(":")[1]
        let neworder=''
        if(flag==initialflag){
          neworder=`${flag}:${initialsort=='desc'?'asc':'desc'}`
        }else{
          neworder=`${flag}:${'desc'}`
        }
